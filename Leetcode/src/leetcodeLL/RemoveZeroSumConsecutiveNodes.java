...
public class RemoveZeroSumConsecutiveNodes {
	...
	public ListNode removeZeroSumSublists(ListNode head) {
		...
		// Thus, here we track the sum from index 0 to each index, using a Map to
		// indicate
		// the farthest index N that we can remove from index M, then we shall be able
		// to
		...
		// Second iteration to re-connect the nodes to the farthest node where the sum
		// stays unchanged
		sum = 0;
		for (ListNode p = preHead; p != null; p = p.next) {
			sum += p.val;
